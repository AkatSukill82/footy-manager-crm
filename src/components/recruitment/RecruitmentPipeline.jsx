...
export default function RecruitmentPipeline({ cases = [], onDelete, onOpen, onSimulate }) {
              ...
              {items.map((c) => {
                ...
                return (
                  <div key={c.id} className="flex items-start gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2">
                    ...
                    {!c.is_minor && onSimulate && (
                      <button onClick={() => onSimulate(c)} title="Simuler le deal (Simulation 360)" className="p-1.5 rounded text-slate-400 hover:text-violet-600 hover:bg-violet-50 flex-shrink-0"><ArrowRightLeft className="w-4 h-4" /></button>
                    )}
