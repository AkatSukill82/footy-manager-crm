...
export default function ClubSearch({ onClose }) {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const data = await TransfermarktAPI.searchClubs(query.trim());
      const list = data?.results || [];

      if (list.length === 0) {
        setError(`Aucun club trouvé pour "${query}" sur Transfermarkt.`);
        setLoading(false);
        return;
      }
  ...
  // ── Étape 2 : profil complet TM + logo API-Football (pas de LLM) ──────────
  const fetchFullProfile = async (tmId, clubName) => {
    setLoadingFull(true);
    setResult(null);
    setCandidates(null);
    setError(null);
    ...
    try {
      // TM profil + AF team en parallèle
      const [tmResult, afResult] = await Promise.allSettled([
        TransfermarktAPI.getClubProfile(tmId),
        base44.functions.invoke("apiFootballProxy", { action: "searchTeam", name: clubName }),
      ]);

      const profile = tmResult.status === "fulfilled" ? tmResult.value : null;
      if (!profile || profile.error) throw new Error("Profil introuvable sur Transfermarkt.");

      // API-Football : prendre le 1er résultat qui correspond le mieux au nom
      const afTeams = afResult.status === "fulfilled" ? (afResult.value?.teams || []) : [];
      const afTeam  = afTeams[0] || null;

      const crmClub = TransfermarktAPI.buildCRMClub(profile, afTeam);
      setResult({ ...crmClub, _tmId: tmId, _afTeamId: afTeam?.id });
    } catch (err) {
      setError(err.message || "Erreur lors du chargement du profil club.");
    } finally {
...
IMPORTANT : retourne uniquement ce que tu sais avec certitude. Si inconnu → null. Ne génère pas de données inventées.

Champs attendus :
- entraineur : entraîneur actuel
- president : président actuel
- directeur_sportif : directeur sportif actuel
- budget_transfert : budget transfert en millions € (number)
- palmares : palmarès majeur (chaîne, titres séparés par virgules)
- historique : courte description du club (2-3 phrases)
- categorie : "Elite" | "Premier plan" | "Intermédiaire" | "En développement"`,
  ...
  const reset = () => { setResult(null); setQuery(""); setSaved(false); setCandidates(null); setError(null); };
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={handleSearch} className="flex gap-2">
        <div className="relative flex-1">
          ...
          <Input
            ...
            placeholder="Ex: Paris Saint-Germain, Manchester City, Real Madrid…"
            className="pl-9"
          />
        </div>
        <Button type="submit" disabled={loading || loadingFull || !query.trim()} className="bg-slate-900 hover:bg-slate-800">
      ...
      {(loading || loadingFull) && (
        <div className="flex items-center justify-center gap-3 py-8 text-slate-500">
          ...
          <span>{loadingFull ? "Chargement du profil club…" : t(lang, 'clubs.searchingTM')}</span>
        </div>
      )}
            ...
            {candidates.map((c, i) => (
              ...
              >
                <div className="w-12 h-12 rounded-xl bg-slate-50 flex-shrink-0 overflow-hidden border border-slate-100 flex items-center justify-center p-1">
                  {c.image
                    ? <img src={c.image} alt={c.name} className="w-full h-full object-contain" referrerPolicy="no-referrer" onError={e => e.target.style.display = 'none'} />
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-1">
                    ...
                    {c.squad?.size && <Badge className="bg-slate-100 text-slate-600 text-xs">{c.squad.size} joueurs</Badge>}
                    {c.marketValues?.avg && <Badge className="bg-green-50 text-green-700 text-xs">{c.marketValues.avg}</Badge>}
                  </div>
                </div>
      ...
      {result && !loading && !loadingFull && (
        <Card className="border-2 border-green-100">
          <CardHeader className="pb-3">
            <div className="flex items-start gap-3">
              <div className="w-16 h-16 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center p-1.5 flex-shrink-0">
                {result.logo_url
                  ? <img src={result.logo_url} alt={result.nom} className="w-full h-full object-contain" referrerPolicy="no-referrer" onError={e => e.target.style.display = 'none'} />
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1 flex-wrap">
                  ...
                  <span className="text-[10px] text-slate-400 border border-slate-200 rounded-full px-2 py-0.5">Transfermarkt</span>
                </div>
              </div>
            </div>
          </CardHeader>
          ...
          <CardContent className="space-y-4">
            ...
            {/* Staff (si enrichissement IA fait) */}
            ...
            {!result.entraineur && !result.palmares && (
              <div className="flex flex-col sm:flex-row items-center gap-3 bg-slate-50 border border-dashed border-slate-200 rounded-lg px-4 py-3">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">Entraîneur, président, palmarès, budget — généré par IA. À vérifier avant utilisation.</p>
                </div>
                <Button onClick={handleEnrichWithAI} disabled={enriching} variant="outline" size="sm"
                  ...
                  {enriching ? "Analyse…" : "Enrichir avec IA"}
                </Button>
              </div>
            )}
