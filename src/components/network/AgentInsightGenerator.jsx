  ...
  const generateInsights = async () => {
    ...
    try {
      ...
      const context = {
        ...
        avg_player_value: players.reduce((sum, pl) => sum + (pl.valeur_marchande || 0), 0) / players.length,
        total_teams: teams.length,
        recent_transfer_value: transfers.slice(0, 10).reduce((sum, tr) => sum + (tr.montant || 0), 0)
      };
      ...
      if (topPlayers.length > 0) {
        const opportunityInsight = await base44.integrations.Core.InvokeLLM({
          ...
          ${topPlayers.map(pl => `- ${pl.nom}, ${pl.age} ans, ${pl.poste}, ${pl.valeur_marchande}M€, ${pl.club_actuel || 'libre'}`).join('\n')}
  ...
  return (
    <Card className="border-2 border-dashed border-slate-300">
      <CardContent className="p-6 text-center">
        ...
        <h3 className="text-lg font-bold text-slate-900 mb-2">{t(lang, 'network.agentTitle')}</h3>
        <p className="text-slate-600 mb-4">{t(lang, 'network.agentDesc')}</p>
        <Button
        ...
        >
          {isGenerating ? (
            <>
              ...
              {t(lang, 'network.analyzing')}
            </>
          ) : (
            <>
              <Sparkles className="w-4 h-4 mr-2" />
              {t(lang, 'network.generateBtn')}
            </>
          )}
        </Button>
      </CardContent>
