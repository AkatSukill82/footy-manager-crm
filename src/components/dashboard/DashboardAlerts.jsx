...
export default function DashboardAlerts({ players = [], commissions = [], reminders = [] }) {
  ...
  const alerts = useMemo(() => {
    ...
    for (const p of players) {
      ...
      if (d == null) continue;
      if (d < 0) list.push({ id: `contract:${p.id}`, sev: "red", title: `Contrat expiré — ${p.nom}`, sub: p.contrat_fin || "", href: createPageUrl("PlayerDetail") + "?id=" + p.id });
      else if (d <= 180) list.push({ id: `contract:${p.id}`, sev: d <= 30 ? "amber" : "blue", title: `Contrat expire dans ${d}j — ${p.nom}`, sub: p.club_actuel || "—", href: createPageUrl("PlayerDetail") + "?id=" + p.id });
    }
    for (const c of commissions) {
      if ((c.sens || "entree") === "sortie" || c.nature === "reel") continue;
      if (c.date_echeance && c.date_echeance < today) list.push({ id: `finance:${c.id}`, sev: "red", title: `Commission en retard — ${c.titre}`, sub: `échéance ${c.date_echeance}`, href: createPageUrl("Finance") });
    }
    for (const r of reminders) {
      if (r.statut === "Terminé") continue;
      const d = daysUntil(r.date_rappel);
      if (d == null) continue;
      if (d < 0) list.push({ id: `reminder:${r.id}`, sev: "red", title: r.titre, sub: "Rappel en retard", href: createPageUrl("Contacts") });
      else if (d === 0) list.push({ id: `reminder:${r.id}`, sev: "amber", title: r.titre, sub: "Rappel aujourd'hui", href: createPageUrl("Contacts") });
    }
  ...
  }, [players, commissions, reminders, state]);
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-xl overflow-hidden">
      ...
      {alerts.length === 0 ? (
        <p className="text-sm text-slate-400 text-center py-5 flex items-center justify-center gap-2"><CheckCircle2 className="w-4 h-4 text-green-500" /> Rien à traiter 🎉</p>
      ) : (
          ...
          {alerts.slice(0, 12).map((a) => (
            <div key={a.id} className="flex items-center gap-2 rounded-lg px-2.5 py-2 hover:bg-slate-50 group">
              ...
              <div className="flex items-center gap-0.5 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                <button onClick={() => snooze(a.id)} title="Reporter 7 jours" className="p-1 rounded text-slate-400 hover:text-amber-600"><BellOff className="w-3.5 h-3.5" /></button>
                <button onClick={() => upd(a.id, "done")} title="Marquer traité" className="p-1 rounded text-slate-400 hover:text-green-600"><CheckCircle2 className="w-3.5 h-3.5" /></button>
              </div>
            </div>
          ))}
        </div>
