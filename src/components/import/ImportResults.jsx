...
import { CheckCircle, AlertCircle, Plus, RefreshCw, RotateCcw } from "lucide-react";
...
export default function ImportResults({ results, onReset }) {
  ...
  const totalSuccess = clubs_crees + clubs_mis_a_jour + contacts_crees + contacts_mis_a_jour;

  const stats = [
    { label: t(lang, "import.clubsCreated"),   value: clubs_crees,      icon: Plus,      color: "text-blue-600",   bg: "bg-blue-100" },
    { label: t(lang, "import.clubsUpdated"),    value: clubs_mis_a_jour, icon: RefreshCw, color: "text-indigo-600", bg: "bg-indigo-100" },
    { label: t(lang, "import.contactsCreated"), value: contacts_crees,   icon: Plus,      color: "text-orange-600", bg: "bg-orange-100" },
    { label: t(lang, "import.contactsUpdated"), value: contacts_mis_a_jour, icon: RefreshCw, color: "text-amber-600", bg: "bg-amber-100" },
  ];
            ...
            <div className="space-y-1 max-h-64 overflow-y-auto">
              {details.map((d, i) => (
                <div key={i} className="flex items-center gap-2 text-sm py-1 border-b border-slate-100 last:border-0">
                  <Badge variant={d.action.startsWith("créé") ? "default" : "secondary"} className="text-xs shrink-0">
                    {d.action}
                  </Badge>
