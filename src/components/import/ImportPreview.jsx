...
const PLAYER_LABELS = {
  ...
  nationalite: "Nationalité", age: "Âge", email: "Email", telephone: "Téléphone",
  whatsapp: "WhatsApp", instagram: "Instagram", twitter: "Twitter",
  agent: "Agent", agent_email: "Email Agent", agent_telephone: "Tel Agent",
  valeur_marchande: "Valeur (M€)", salaire: "Salaire (M€/an)",
  contrat_fin: "Fin contrat", taille: "Taille (cm)", poids: "Poids (kg)",
  ligue: "Ligue", buts: "Buts", passes_decisives: "Passes déc.", matchs_joues: "Matchs"
};

const CLUB_LABELS = {
  nom: "Nom club", pays: "Pays", ville: "Ville", stade: "Stade",
...
function DataTable({ items, labels, title, icon: Icon, color }) {
  ...
  const usedKeys = Object.keys(labels).filter(k => items.some(item => item[k] != null && item[k] !== ""));
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-slate-200">
                  {usedKeys.map(k => (
                    <th key={k} className="text-left py-2 px-2 text-slate-500 font-medium whitespace-nowrap">{labels[k]}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {items.map((item, i) => (
                  <tr key={i} className={i % 2 === 0 ? "bg-slate-50" : ""}>
                    {usedKeys.map(k => (
                      <td key={k} className="py-2 px-2 text-slate-700 whitespace-nowrap max-w-[150px] truncate">
                        {item[k] != null ? String(item[k]) : <span className="text-slate-300">—</span>}
                      </td>
                    ))}
                  </tr>
                ))}
